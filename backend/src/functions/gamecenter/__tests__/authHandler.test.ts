...
describe("Game Center Authentication Handler", () => {
  ...
  describe("Token Validation", () => {
    it("should reject requests without token", async () => {
      ...
      expect(JSON.parse(result.body)).toEqual({
        error: "Game Center token is required",
      });
    });
    ...
    it("should reject invalid token format", async () => {
      ...
      expect(JSON.parse(result.body)).toEqual({
        error: "Invalid Game Center token",
      });
    });
  ...
  describe("User Management", () => {
    it("should return existing user if found", async () => {
      ...
      const validToken = Buffer.from(JSON.stringify(validTokenData)).toString(
        "base64",
      );
  ...
  describe("Security Tests", () => {
    ...
    it("should validate all required token fields", async () => {
      ...
      const incompleteToken = Buffer.from(
        JSON.stringify(incompleteTokenData),
      ).toString("base64");
    ...
    it("should enforce reasonable timestamp limits", async () => {
      ...
      const futureToken = Buffer.from(JSON.stringify(futureTokenData)).toString(
        "base64",
      );
