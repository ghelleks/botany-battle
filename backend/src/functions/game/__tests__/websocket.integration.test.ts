...
describe('WebSocket Functionality Integration Tests', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    mockWebSocket = new WebSocket('ws://localhost:3001') as jest.Mocked<WebSocket>;
  });
  ...
  describe('Connection Management', () => {
    ...
    it('should manage multiple concurrent connections', async () => {
      const connections = [];
      for (let i = 0; i < 100; i++) {
        const connection = new WebSocket('ws://localhost:3001') as jest.Mocked<WebSocket>;
  ...
  describe('Error Handling and Recovery', () => {
    ...
    it('should attempt reconnection on connection loss', async () => {
      ...
      for (let i = 0; i < 3; i++) {
        const newConnection = new WebSocket('ws://localhost:3001') as jest.Mocked<WebSocket>;
  ...
  describe('Game State Synchronization', () => {
    it('should keep game state synchronized between players', async () => {
      const player1Socket = new WebSocket('ws://localhost:3001') as jest.Mocked<WebSocket>;
      const player2Socket = new WebSocket('ws://localhost:3001') as jest.Mocked<WebSocket>;
