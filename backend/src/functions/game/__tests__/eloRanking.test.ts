...
describe('ELO Ranking System', () => {
  ...
  describe('getMatchmakingRange', () => {
    ...
    it('should expand range for longer wait times', () => {
      ...
      expect(longRange.max - longRange.min).toBeGreaterThan(shortRange.max - shortRange.min);
    });
  ...
  describe('Integration Tests', () => {
    it('should maintain rating conservation in head-to-head matches', () => {
      ...
      const totalChange = results.winner.ratingChange + Math.abs(results.loser.ratingChange);
    ...
    it('should handle rapid rating progression for new players', () => {
      ...
      for (let i = 0; i < 10; i++) {
        const change = calculateELOChange(currentRating, baseOpponentRating, 1, i);
    ...
    it('should stabilize ratings for experienced players', () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        const change = calculateELOChange(currentRating, opponentRating, result, 100 + i);
        currentRating += change;
      }
