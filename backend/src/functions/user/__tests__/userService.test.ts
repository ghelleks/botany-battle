...
describe("User Service", () => {
  ...
  describe("getUserStats", () => {
    it("should return user achievements", async () => {
      const mockStats = {
        ...
        plantsIdentified: 75,
        longestStreak: 8,
        rankHistory: [{ rank: "Sprout", achievedAt: "2023-01-01T00:00:00Z" }],
  ...
  describe("updateUserELO", () => {
    it("should update user ELO and stats", async () => {
      ...
      expect(updateCall.input.ExpressionAttributeValues[":eloRating"]).toBe(
        1050,
      );
    ...
    it("should handle rank changes", async () => {
      ...
      const updateCall = mockSend.mock.calls[1][0];
      const rankHistory =
        updateCall.input.ExpressionAttributeValues[":rankHistory"];
    ...
    it("should update win/loss streaks correctly", async () => {
      ...
      const updateCall = mockSend.mock.calls[1][0];
      expect(updateCall.input.ExpressionAttributeValues[":currentStreak"]).toBe(
        4,
      );
    });
  ...
  describe("Achievement System", () => {
    it("should unlock achievements based on user stats", async () => {
      ...
      const plantNovice = achievements.find(
        (a: any) => a.id === "plant_novice",
      );
      expect(plantNovice.isUnlocked).toBe(true);
      expect(plantNovice.progress).toBe(50); // Capped at maxProgress

      const streakMaster = achievements.find(
        (a: any) => a.id === "streak_master",
      );
      expect(streakMaster.isUnlocked).toBe(true);

      const greenThumbRank = achievements.find(
        (a: any) => a.id === "rank_green_thumb",
      );
