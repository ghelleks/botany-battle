 ...
 */
export async function getLeaderboard(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
 ...
 */
export async function getUserStats(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  try {
    const userId = event.requestContext.authorizer?.claims?.sub;
    if (!userId) {
      return {
...
export async function updateUserELO(
  ...
  try {
    ...
    const longestStreak = Math.max(stats.longestStreak, Math.abs(currentStreak));
...
function generateAchievements(stats: UserStats) {
  ...
  achievements.push({
    ...
    unlockedDate: stats.totalWins >= 1 ? stats.rankHistory[0]?.achievedAt : null,
  ...
  const rankAchievements = [
    { rank: 'Sprout', title: 'Growing Strong', description: 'Reach Sprout rank' },
    { rank: 'Green Thumb', title: 'Green Thumb', description: 'Reach Green Thumb rank' },
    { rank: 'Gardener', title: 'Master Gardener', description: 'Reach Gardener rank' },
    { rank: 'Botanist', title: 'Professional Botanist', description: 'Reach Botanist rank' }
  ];
  ...
  for (const rankAch of rankAchievements) {
    ...
    achievements.push({
      ...
      unlockedDate: hasReached ? stats.rankHistory.find(r => r.rank === rankAch.rank)?.achievedAt : null,
