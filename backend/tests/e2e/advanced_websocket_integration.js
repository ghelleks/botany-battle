...
class AdvancedWebSocketTester {
    ...
    async testMultiplayerGameFlow() {
        ...
        try {
            ...
            for (let round = 1; round <= 5; round++) {
                ...
                if (gameState1.data.round !== round || gameState2.data.round !== round) {
    ...
    async testConcurrentConnections() {
        ...
        try {
            ...
            console.log(`Concurrent connections: ${successfulConnections}/${connectionCount} (${(successRate * 100).toFixed(1)}%)`);
    ...
    async testMalformedMessageHandling() {
        ...
        try {
            ...
            console.log(`Sent ${malformedMessages.length} malformed messages, received ${errorResponseCount} error responses`);
    ...
    async testGameStateConsistency() {
        ...
        try {
            ...
            const consistent = (
                ...
                JSON.stringify(gameState1.data.plant) === JSON.stringify(gameState2.data.plant) &&
    ...
    async runAllTests() {
        ...
        const tests = [
            { name: 'Multiplayer Game Flow', test: () => this.testMultiplayerGameFlow() },
            { name: 'Connection Recovery', test: () => this.testConnectionRecovery() },
            { name: 'High Frequency Messages', test: () => this.testHighFrequencyMessages() },
            { name: 'Concurrent Connections', test: () => this.testConcurrentConnections() },
            { name: 'Malformed Message Handling', test: () => this.testMalformedMessageHandling() },
            { name: 'Game State Consistency', test: () => this.testGameStateConsistency() }
        ];
    ...
    generateReport(results) {
        ...
        console.log('-'.repeat(60));
        console.log(`Total: ${passedTests}/${totalTests} tests passed (${successRate.toFixed(1)}%)`);
